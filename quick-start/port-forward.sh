...
main() {
    ...
    # Port forward OpenTelemetry Collector (21893)
    log_info "Setting up OpenTelemetry Collector port forwarding (21893)..."
    if nodeport_otel_collector=$(get_nodeport "opentelemetry-collector" "$OBSERVABILITY_NS"); then
        setup_port_forward 21893 "$nodeport_otel_collector" "OpenTelemetry Collector"
    else
        log_warning "Skipping OpenTelemetry Collector (service not found or not NodePort type)"
    fi
    echo "" >&2
    ...
    echo "  • OpenTelemetry Collector:      http://localhost:21893" >&2
