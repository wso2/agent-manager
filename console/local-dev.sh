...
find workspaces/libs -name "tsconfig.lib.json" -type f 2>/dev/null | while read tsconfig; do
...
find workspaces/pages -name "tsconfig.lib.json" -type f 2>/dev/null | while read tsconfig; do
  pkg_dir=$(dirname "$tsconfig")
  pkg_name=$(basename "$pkg_dir")
