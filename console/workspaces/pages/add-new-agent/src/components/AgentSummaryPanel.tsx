...
import { Close, RocketOutlined, Add } from "@mui/icons-material";
...
export const AgentSummaryPanel = (
    { control, errors, isValid, isPending, onCancel, onSubmit, mode = 'deploy' }: SummaryPanelProps
) => {
    ...
    const errorMessages = useMemo(() => {
        ...
        errors.port?.message,
        errors.openApiFileName?.message,
        errors.openApiContent?.message
    ]);
        ...
        <Container maxWidth="lg" sx={{ display: 'flex', justifyContent: 'space-between', gap: 1 }}>
            <Box display="flex" flexDirection="column" gap={0.5}>
                <Typography variant="h6">
                    {agentName} {isConnectMode ? '' : '→ Development'}
                </Typography>
                <Typography variant="caption" color="text.secondary">
                    {isConnectMode
                        ? 'External agent with OpenTelemetry integration'
            ...
            <Box display="flex" flexDirection="row" gap={1} alignItems="center">
                ...
                <Button
                    ...
                    color="primary"
                    size='large'
                    startIcon={isConnectMode ? <Add fontSize="small" /> : <RocketOutlined fontSize="small" />}
                    onClick={onSubmit}
                    disabled={isPending || !isValid}
                >
                    {isConnectMode ? 'Create Agent' : 'Deploy to Development'}
                </Button>
            </Box>
        </Container>
    );
