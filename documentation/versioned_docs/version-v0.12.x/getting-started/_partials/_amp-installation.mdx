...
:::info Prerequisites
Thunder (identity provider) was installed in [Phase 1 Step 4](#step-4-install-thunder-extension-identity-provider). The variables `THUNDER_PUBLIC_URL`, `THUNDER_INTERNAL_URL`, `CONSOLE_PUBLIC_URL`, `API_PUBLIC_URL`, `OBS_API_PUBLIC_URL`, and `INSTRUMENTATION_URL` must be set from the [Configuration Variables](#configuration-variables) section.
:::
...
```bash
kubectl apply -f https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.12.0/deployments/values/obs-gateway.yaml

kubectl wait --for=condition=Programmed \
  apigateway/obs-gateway -n ${DATA_PLANE_NS} --timeout=180s

kubectl apply -f https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.12.0/deployments/values/otel-collector-rest-api.yaml
