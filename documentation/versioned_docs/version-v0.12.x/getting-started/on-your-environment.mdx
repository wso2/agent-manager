...
Set these once before starting the installation. All subsequent commands reference these variables.
...
kubectl exec -n ${THUNDER_NS} deploy/amp-thunder-extension-deployment -- \
  ...
  | grep -o '"issuer":"[^"]*"'
# Expected: "issuer":"http://localhost:8090"  (matching THUNDER_PUBLIC_URL)
```
:::
...
security:
  oidc:
    ...
    tokenUrl: "${THUNDER_PUBLIC_URL}/oauth2/token"
gateway:
  tls:
    enabled: true
...
```bash
kubectl apply -f https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.12.0/deployments/values/api-platform-operator-full-config.yaml
```
