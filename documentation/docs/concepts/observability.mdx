...
- **Manual instrumentation** for agents on a custom or non-frontier framework. You emit spans yourself against a published contract (OpenTelemetry GenAI semantic conventions). Spans that follow the contract render with the full trace view and run through evaluators; spans that don't still appear, just without the rich view. See the [manual instrumentation guide](./manual-instrumentation.mdx).
