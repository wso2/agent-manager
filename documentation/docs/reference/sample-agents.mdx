...
:::warning CrewAI needs a writable home directory
Set `HOME=/tmp` and `CREWAI_STORAGE_DIR=/tmp/crewai` as environment variables on the agent. CrewAI writes under `$HOME` when it is imported, before your code runs, and the build container's default home is not writable. Without these the agent fails at startup rather than at request time.
:::
