...
| `console.config.fileMountMaxFileBytes` | Per-file cap, in bytes, on file-mount content in the console. 0 uses agentManagerService.config.fileMountLimits.maxFileBytes, keeping the two in step. The console also holds it under maxRequestBodyBytes (less 8 KB for the rest of the save), so with the defaults the effective cap is 48 KB; set maxRequestBodyBytes to 0 to allow files up to this value. | integer | `0` |
