...
| `console.config.docsUrl` | Full documentation base URL, overriding docsVersion. Empty builds the URL from docsVersion, which is what a versioned release wants. Set it for a deployment that tracks one continuously-updated set of docs rather than a release's own snapshot - cloud, which is served from /docs/cloud/ and is never version-stamped. Because this wins over docsVersion, release version stamping cannot reach a deployment that sets it. No trailing slash; the console appends the page path. | string | `""` |
