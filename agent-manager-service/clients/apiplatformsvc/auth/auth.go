...
// Package auth provides authentication for API Platform API.
package auth
...
// AuthProvider implements client.AuthProvider using OAuth2 client credentials
...
func NewAuthProvider(cfg Config) client.AuthProvider {
	return &AuthProvider{
		config:     cfg,
		httpClient: requests.NewRetryableHTTPClient(&http.Client{}, requests.RequestRetryConfig{}),
	}
}
...
func (p *AuthProvider) GetToken(ctx context.Context) (string, error) {
	...
	slog.Debug("apiplatform auth: fetching new token")
	...
	slog.Info("apiplatform auth: fetched new access token",
...
func (p *AuthProvider) fetchToken(ctx context.Context) (string, int64, error) {
	req := &requests.HttpRequest{
		Name:   "apiplatform.auth.fetchToken",
	...
	if err := requests.SendRequest(ctx, p.httpClient, req).ScanResponse(&tokenResp, http.StatusOK); err != nil {
		return "", 0, fmt.Errorf("apiplatform.auth.fetchToken: %w", err)
	}
