	...
	Migrate: func(db *gorm.DB) error {
		createArtifactsAndAPIsSQL := `
			...
			CREATE TABLE artifacts (
				...
				CONSTRAINT uq_artifact_handle_org UNIQUE(handle, organization_name),
				CONSTRAINT uq_artifact_name_version_org UNIQUE(name, version, organization_name)
			);
			...
			CREATE TABLE deployment_status (
				...
				CONSTRAINT pk_deployment_status PRIMARY KEY (artifact_uuid, organization_name, gateway_uuid),
			...
			CREATE TABLE association_mappings (
				...
				CONSTRAINT fk_association_artifact FOREIGN KEY (artifact_uuid)
					REFERENCES artifacts(uuid) ON DELETE CASCADE,
				CONSTRAINT uq_association_artifact_resource_type UNIQUE(artifact_uuid, resource_uuid, association_type, organization_name),
				CONSTRAINT chk_association_type CHECK (association_type IN ('gateway', 'dev_portal'))
			);

			-- Indexes for performance
			CREATE INDEX idx_artifacts_org ON artifacts(organization_name);
			...
			CREATE INDEX idx_deployments_org_gateway_created ON deployments(artifact_uuid, organization_name, gateway_uuid, created_at DESC);
			...
			CREATE INDEX idx_association_artifact_resource_type ON association_mappings(artifact_uuid, association_type, organization_name);
			CREATE INDEX idx_association_resource ON association_mappings(association_type, resource_uuid, organization_name);
			CREATE INDEX idx_association_org ON association_mappings(organization_name);
		`
