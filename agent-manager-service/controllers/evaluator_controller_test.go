...
func TestMain(m *testing.M) {
	// Set required environment variables for config loader
	_ = os.Setenv("DB_HOST", "localhost")
	_ = os.Setenv("DB_USER", "testuser")
	_ = os.Setenv("DB_PASSWORD", "testpass")
	_ = os.Setenv("DB_NAME", "testdb")
	_ = os.Setenv("OPEN_CHOREO_BASE_URL", "http://localhost:8080")
...
// Helper function to create mock evaluator responses
func createMockEvaluators() []*models.EvaluatorResponse {
	...
	return []*models.EvaluatorResponse{
		...
		},
		{
			...
			Version:     "1.0",
			Provider:    "deepeval",
			Tags:        []string{"deepeval", "agent", "action", "tool-use"},
			IsBuiltin:   true,
			ConfigSchema: []models.EvaluatorConfigParam{
				{
					Key:         "threshold",
...
func TestListEvaluators_Success(t *testing.T) {
	...
	controller := NewEvaluatorController(mockService)
...
func TestListEvaluators_WithTagsFilter(t *testing.T) {
	...
	taggedEvaluators := []*models.EvaluatorResponse{
		createMockEvaluators()[1], // has deepeval and tool-use tags
	}

	mockService.On("ListEvaluators", mock.Anything, (*uuid.UUID)(nil), services.EvaluatorFilters{
		Limit:    20,
		Offset:   0,
		Tags:     []string{"deepeval", "tool-use"},
		Search:   "",
		Provider: "",
