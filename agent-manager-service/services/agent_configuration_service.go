...
func (s *agentConfigurationService) processNewEnv(
	...
	rbRes := rollbackResource{proxyHandle: proxy.Handle, providerAPIKeyID: providerAPIKeyID, providerUUID: providerUUID, providerSecretPath: providerSecretPath}
...
func (s *agentConfigurationService) Update(ctx context.Context, configUUID uuid.UUID, orgName, projectName, agentName string,
	...
	if len(req.EnvironmentVariables) > 0 {
		// Load persisted names so validation uses the actual current state, not autogenerated defaults.
		persistedVarNames, err := s.loadExistingVarNames(ctx, configUUID)
		if err != nil {
			return nil, err
		}
		...
		for _, ev := range req.EnvironmentVariables {
			// Overlay the requested rename on top of the persisted value.
			found := false
		...
		// Validate using the merged result (catches uniqueness and format errors against real persisted names).
		...
		if hasExisting {
			...
			if providerChanged {
				...
				oldHandle, rbRes, err := s.processEnvProviderChange(
					ctx, configUUID, existingConfig, env, envUUID, envName, envMapping, existingMapping, orgName, existingVarNames)
				if err != nil {
					s.rollbackProxies(ctx, rollbackResources, orgName)
					return nil, err
				}
			...
			rbRes, err := s.processNewEnv(
				ctx, configUUID, existingConfig, env, envUUID, envName, envMapping, orgName, existingVarNames)
			if err != nil {
				s.rollbackProxies(ctx, rollbackResources, orgName)
				return nil, err
			}
...
// Returns the mapping from the first environment found (names are config-level).
func (s *agentConfigurationService) loadExistingVarNames(ctx context.Context, configUUID uuid.UUID) (map[string]string, error) {
	...
	for _, v := range vars {
		// Last-write wins; since names are config-level, all envs should have same names
		if _, already := result[v.VariableKey]; !already {
