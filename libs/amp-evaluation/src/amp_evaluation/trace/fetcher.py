...
These traces can then be parsed into EvalTrace objects for evaluation.
...
class Trace:
    ...
    Can be converted to EvalTrace using parse_trace_for_evaluation().
    """
    traceId: str
    rootSpanId: str
    rootSpanName: str
...
class TraceFetcher:
    ...
    EvalTrace objects using parse_trace_for_evaluation().
    ...
    def fetch_traces(
        ...
        limit: int = 100,
        offset: int = 0,
        agent_id: Optional[str] = None  # Ignored, uses agent_uid from __init__
    ) -> List[Trace]:
        """
        ...
        Args:
            ...
            agent_id: Ignored (agent_uid from __init__ is used instead)
    ...
    def load_batch(
        ...
        if start_time or end_time:
            filtered_traces = [
                ...
                if self._matches_time_filter(t, start_time, end_time)
            ]
