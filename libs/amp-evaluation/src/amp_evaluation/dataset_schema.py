...
class DatasetTask:
    ...
    Required fields:
        ...
        - input: The user query/prompt (string or structured dict for JSON payloads)
    ...
    # Required
    id: str
    input: Union[str, Dict[str, Any]]  # String or structured JSON input
...
class DatasetSchema:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"name": self.name, "schema_version": self.schema_version, "tasks": []}
        ...
        for task in self.tasks:
            task_dict = {"id": task.id, "input": task.input}
            if task.name:
...
def from_csv(csv_path: str, name: Optional[str] = None) -> DatasetSchema:
    ...
    return DatasetSchema(name=name or path.stem, tasks=tasks, description=f"Loaded from {path.name}")
