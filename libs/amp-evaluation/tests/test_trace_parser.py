...
Tests parsing raw OTEL/AMP traces into EvalTrace format.
"""
import pytest
import sys
import json
...
def dict_to_otel_trace(trace_dict: dict) -> 'OTELTrace':
    ...
    for span_dict in trace_dict.get('spans', []):
        ...
        if has_error:
            error_count += 1
            amp_attrs['error'] = {
                'message': span_dict.get('status', {}).get('errorType', 'Error')
            }
...
class TestTraceParser:
    ...
    def test_convenience_properties(self):
        """Test EvalTrace convenience properties."""
        raw_trace_dict = {
        ...
        # Test properties
        assert eval_trace.has_output == True
        assert eval_trace.has_errors == False
...
class TestEvalTraceStructure:
    """Test the EvalTrace data structure itself."""
...
class TestRealOTELTraces:
    ...
    def test_chain_spans_are_skipped(self, sample_traces):
        ...
        for eval_trace in eval_traces:
            # EvalTrace only has llm_spans, tool_spans, retriever_spans, agent_span
